...
def trainCNNTransformer():
    ...
    transformer_model.add(InputLayer(input_shape=(X_train.shape[1], X_train.shape[2], X_train.shape[3])))
    transformer_model.add(Conv2D(64, (5, 5), activation='relu', strides=(1, 1), padding='same'))
    ...
    if os.path.exists("model/transformer_weights.hdf5") == False:
        transformer_model = Sequential()
        transformer_model.add(MultiHeadAttention(InputLayer(input_shape=(X_train.shape[1], X_train.shape[2], X_train.shape[3]))))
    ...
    predict = np.argmax(predict, axis=1)
    y_test1 = np.argmax(y_test, axis=1)
    calculateMetrics("Proposed tranformer", predict, y_test1)#call function to calculate accuracy and other metrics
...
vggButton = Button(main, text="Trin VGG (Existing) Algorithm", command=trainVGG)
